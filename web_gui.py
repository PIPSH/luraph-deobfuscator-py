...
class WebGuiHandler(BaseHTTPRequestHandler):
    ...
    @staticmethod
    def _infer_warning(deobfuscated: str, stdout: str, stderr: str) -> str:
        combined = " ".join([deobfuscated or "", stdout or "", stderr or ""]).lower()
        if "script key required" in combined or "script key missing" in combined:
            return "The deobfuscator could not find the script key; output may be incomplete."
        if "decoded_chunk" in combined or "placeholder" in combined:
            return "Output still contains placeholder chunks; a script key or sandbox capture may be required."
        return ""
    ...
    def do_POST(self) -> None:  # noqa: N802 - http.server naming
        ...
        warning = self._infer_warning(deobfuscated, result.stdout, result.stderr)
        response = {
